...
function pathFor(matterId: string, notice?: string): string {
  const base = `/app/matters/${matterId}/security`;
...
export async function recordSecurityAction(formData: FormData): Promise<void> {
  const matterId = String(formData.get("matterId") ?? "");
  await act(matterId, async (ctx) => {
    await recordSecurityCheck(db(), ctx, matterId);
    return "Logical readback recorded. This is not a production restore.";
  });
}
