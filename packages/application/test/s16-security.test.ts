...
run("S16 security", () => {
  ...
  beforeAll(async () => {
    ...
    matterId = (
      await createMatter(app, ctxA, { title: "Security", idea: "A worker records a timeout." })
    ).id;
  });
  ...
  it("reads the matter back without certifying a production restore", async () => {
    const recorded = await recordSecurityCheck(app, ctxA, matterId);
    expect(recorded.matterTitle).toBe("Security");
    expect(recorded.observation.certified).toBe(false);
    const detail = recorded.observation.detail as {
      productionRestore: boolean;
      assertionCount: number;
    };
    expect(detail.productionRestore).toBe(false);
    expect(detail.assertionCount).toBeGreaterThan(0);
    expect(recorded.unresolved.join(" ")).toMatch(/not a certified/);
    ...
    expect(policy?.trainingUseAllowed).not.toBe(true);
    expect(await getSecurity(app, ctxB, matterId)).toBeNull();
  });
});
