...
run("S17 benchmark", () => {
  ...
  beforeAll(async () => {
    ...
    matterId = (
      await createMatter(app, ctxA, { title: "Benchmark", idea: "A worker records a timeout." })
    ).id;
  });
  ...
  it("catches the known defect families without certifying quality", async () => {
    const report = practitionerBenchmark();
    expect(report.checks.every((check) => check.caught)).toBe(true);
    expect(report.practitioners).toBe(0);
    expect(report.statisticalSufficiency).toBe(false);
    expect(report.certifiesPatentability).toBe(false);
    expect(report.deploymentUrl).toBeNull();
    expect(report.limitations.join(" ")).toMatch(/did not independently annotate/);
    const stored = await recordBenchmark(app, ctxA, matterId);
    expect(stored.certified).toBe(false);
    expect(await getBenchmark(app, ctxB, matterId)).toBeNull();
  });
});
